...
{r, include = FALSE} library(tidyverse) library(nflfastR) library(ggimage) library(gt) library(ggthemes) library(ggrepel) library(plotly) library(vip)
...
{r, echo = FALSE} \# Sample offense and defense data (replace with your actual data) offense \<- offense %\>% filter(!is.na(posteam) & posteam != "") defense \<- defense %\>% filter(!is.na(defteam) & defteam != "") off_epa \<- offense$off_epa def_epa <- defense$def_epa team_names \<- offense\$posteam \# Assuming this column contains team abbreviations
...
{r, echo = FALSE} \## Filtering Data for future plots first_pbp \<- load_pbp(1999:2010) second_pbp \<- load_pbp(2011:2024)
...
{r, echo = FALSE} ydstogo_summary \<- fourth_downs_first %\>% group_by(ydstogo) %\>% summarize(count = n(), went_for_it_rate = mean(went_for_it)) %\>% filter(count \>= 5)
...
{r, echo = FALSE} yardline_summary \<- fourth_downs_first %\>% group_by(yardline_100) %\>% summarize(count = n(), went_for_it_rate = mean(went_for_it)) %\>% filter(count \>= 5)
...
{r, echo = FALSE} \# Summarize the data wp_summary \<- fourth_downs_first %\>% mutate(wp_rounded = round(wp, 2)) %\>% group_by(wp_rounded) %\>% summarize(count = n(), went_for_it_rate = mean(went_for_it))
...
{r, echo = FALSE} log_fourth \<- glm(went_for_it \~ yardline_100 + ydstogo + wp, data = fourth_downs_first)
...
{r, include = FALSE} fourth_downs_first$pred_prob <- log_fourth$fitted.values
...
{r, include = FALSE} \## 4th down Modeling fourth_downs_first \<- fourth_downs_first \|\> mutate(pred_prob = log_fourth\$fitted.values) \|\> mutate(fourth_oe = went_for_it - pred_prob)
