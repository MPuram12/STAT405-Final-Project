...
#####CHART 1#####
epa_chart1 <- offense1 %>%
  dplyr::inner_join(defense1, by = c("posteam" = "defteam")) %>%
  dplyr::inner_join(logos, by = c("posteam" = "team_abbr")) %>%
  ggplot2::ggplot(aes(x = off_epa, y = def_epa, alpha = 0.2)) +
  ggplot2::geom_abline(slope = -1.5, intercept = c(.4, .3, .2, .1, 0, -.1, -.2, -.3), alpha = .2) +
  ggplot2::geom_hline(aes(yintercept = mean(off_epa)), color = "red", linetype = "dashed") +
  ...
  labs(
    ...
    y = "Defense EPA/play",
    caption = "Data: @nflfastR",
    title = "NFL Offensive and Defensive EPA per Play 1999-2010"
  ) +
  ggplot2::theme(
    aspect.ratio = 9 / 16,
    plot.title = ggplot2::element_text(size = 12, hjust = 0.5, face = "bold")
...
####CHART2#####
epa_chart2 <- offense2 %>%
  dplyr::inner_join(defense2, by = c("posteam" = "defteam")) %>%
  dplyr::inner_join(logos, by = c("posteam" = "team_abbr")) %>%
  ggplot2::ggplot(aes(x = off_epa, y = def_epa, alpha = 0.2)) +
  ggplot2::geom_abline(slope = -1.5, intercept = c(.4, .3, .2, .1, 0, -.1, -.2, -.3), alpha = .2) +
  ggplot2::geom_hline(aes(yintercept = mean(off_epa)), color = "red", linetype = "dashed") +
  ...
  labs(
    ...
    y = "Defense EPA/play",
    caption = "Data: @nflfastR",
    title = "NFL Offensive and Defensive EPA per Play 2011-2024"
  ) +
  ggplot2::theme(
    aspect.ratio = 9 / 16,
    plot.title = ggplot2::element_text(size = 12, hjust = 0.5, face = "bold")
...
full_names <- c(
  ...
  "S.Davis" = "Stephen Davis",
  "C.Dillon" = "Corey Dillon",
  "D.McAllister" = "Deuce McAllister"
)
...
ggplot(ryoe_by_player, aes(y = reorder(rusher_player_name, exp_ypc))) + 
  ...
  theme(axis.title = element_text()) +
  labs(title = "Actual YPC vs Expected YPC",
...
**Punt Chart** This graph represents the percentage of punts in the NFL that were fair caught or returned, segmented by return yardage categories, over a span of years from 1999 to 2024. The various colored bands represent different return outcomes: punts returned for 0-10 yards, 11-30 yards, 31-50 yards, and those returned for over 51 yards. The data is stacked, showing the total percentage of all outcomes across time.
...
punt_data <- long_pbp %>%
  ...
  mutate(
    year = substr(game_id, 1, 4), 
    year = as.numeric(year), 
    return_distance = case_when(
...
ggplot(punt_summary_area, aes(x = as.numeric(year), y = percentage, fill = category)) +
  ...
  theme_fivethirtyeight() +
  theme(axis.title = element_text()) +
  labs(title = "% of Fair Caught and Returned Punts (1999-2024)",
...
The largest portion of returned punts consistently falls within the 0-10 yard range, as indicated by the dominant green area across all years. This suggests that the majority of punt returns result in minimal yardage gains. The orange band, representing returns between 11-30 yards, shows some fluctuation but remains stable over the years. Categories for longer returns (31-50 and 51+ yards) are relatively thin, reflecting their rarity. Interestingly, there is minimal variation in the distribution of punt return outcomes over time. This could imply that while NFL rules and strategies may have evolved, the outcomes in punt returns have remained relatively stable. However, it also shows that fair catches and short returns dominate this aspect of special teams play. Overall, the graph illustrates that punt returns with larger gains (beyond 30 yards) are uncommon, and the bulk of punts result in either short returns or fair catches.
