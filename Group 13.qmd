...
qb_02 <- get_qb_stats(2002)
qb_03 <- get_qb_stats(2003)
qb_04 <- get_qb_stats(2004)
qb_05 <- get_qb_stats(2005)
qb_06 <- get_qb_stats(2006)
qb_07 <- get_qb_stats(2007)
qb_08 <- get_qb_stats(2008)
qb_09 <- get_qb_stats(2009)
qb_10 <- get_qb_stats(2010)
qb_11 <- get_qb_stats(2011)
qb_12 <- get_qb_stats(2012)
qb_13 <- get_qb_stats(2013)
qb_14 <- get_qb_stats(2014)
qb_15 <- get_qb_stats(2015)
qb_16 <- get_qb_stats(2016)
qb_17 <- get_qb_stats(2017)
qb_18 <- get_qb_stats(2018)
qb_19 <- get_qb_stats(2019)
qb_20 <- get_qb_stats(2020)
qb_21 <- get_qb_stats(2021)
qb_22 <- get_qb_stats(2022)

players <- c("T.Brady", "P.Manning")

everything <- rbind(qb_02, qb_03, qb_04, qb_05, qb_06, qb_07, qb_08, qb_09, qb_10, qb_11, qb_12, qb_13, qb_14, qb_15, qb_16, qb_17, qb_18, qb_19, qb_20, qb_21, qb_22) %>% 
  mutate(highlighted =  ifelse(name %in% players, "Yes", "No"),
         label = ifelse(Season == 2022, name, NA)) %>% 
  select(name, epa, Season, highlighted, n_dropbacks, label)

# Ensure proper grouping and summarizing of data
averagePlayer <- everything %>%
  group_by(Season) %>%
  dplyr::summarize(epa = mean(epa, na.rm = TRUE)) %>%
  mutate(name = "Average Player",
         highlighted = "Yes",
         n_dropbacks = NA,
         label = ifelse(Season == 2022 | name == "P.Manning", name, NA)
  ) %>%
  select(name, epa, highlighted, Season, n_dropbacks)


everything %>% 
  filter(highlighted == "Yes") %>% 
  ggplot(aes(x = Season, y = epa, group = name)) +
  geom_line(data = everything %>% 
      filter(highlighted == "No", n_dropbacks > 150),
      color = "grey75",
      size = .6,
      alpha = .5) +
  geom_line(aes(color = name), size = 0.6) +
  geom_line(data = averagePlayer, color = "black") +
  geom_text_repel(aes(color = name, label = label),
    fontface = "bold",
    size = 3.5,
    direction = "y",
    xlim = c(2022.2, NA),
    ...
    segment.angle = 20) +
  # geom_text_repel(
  #   data = averagePlayer, aes(label = label),
  #   fontface = "bold",
  #   nudge_y = 1, 
  #   size = 3.5,
  #   direction = "y",
  ...
  scale_x_continuous(
    expand = c(0, 0),
    limits = c(2002.0, 2022.5),
    breaks = seq(2002, 2022, by = 1))+
  scale_color_manual(values = c("#00529F", "#A50044")) +
  xlim(2002, 2022.6) +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 10),
        axis.title.x = element_text(size = 15, hjust = 0.44),
        axis.text.y = element_text(size = 10),
        axis.title.y = element_text(size = 15))+
  labs(
    titles = "QB EPA Year by Year (2003 - 2022)",
    subtitle = "Minimum 150 Dropbacks", 
    x = "Season",
    y = "EPA"
  )
